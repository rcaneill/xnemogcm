...
def open_domain_cfg(datadir='.', load_from_saved=True, save=True, saving_name='xnemogcm.domcfg.nc',
    ...
    #
    domcfg.coords['x_c'] = (['x_c',], domcfg.coords['x_c'], {'axis': 'X'})                               # center point
    domcfg.coords['x_f'] = (['x_f',], domcfg.coords['x_f'], {'axis': 'X', 'c_grid_axis_shift':  0.5})    # right  point
    domcfg.coords['y_c'] = (['y_c',], domcfg.coords['y_c'], {'axis': 'Y'})                               # center point
    domcfg.coords['y_f'] = (['y_f',], domcfg.coords['y_f'], {'axis': 'Y', 'c_grid_axis_shift':  0.5})    # right  point
    domcfg.coords['z_c'] = (['z_c',], domcfg.coords['z_c'], {'axis': 'Z'})                               # center point
    domcfg.coords['z_f'] = (['z_f',], domcfg.coords['z_f'], {'axis': 'Z', 'c_grid_axis_shift': -0.5})    # left   point
    #
    ...
    coordinates = [key for key in domcfg.coords.keys()]    # all coordinates, including unused ones
    ...
    if mercator_grid:
        ...
        for point in ['t', 'u', 'v', 'f']:
            domcfg['x_{}_plot'.format(point.upper())] = (domcfg['glam{}'.format(point)] - np.mean(domcfg['glam{}'.format(point)])) \
                                                        * xr.ufuncs.cos(xr.ufuncs.deg2rad(domcfg['gphi{}'.format(point)])) \
                                                        + np.mean(domcfg['glam{}'.format(point)])
...
def compute_metric_pos(domcfg, metric_pos={}, metric_diff={}):
    ...
    for coord_nme in metric_diff_rename.keys():
        raise(NotImplementedError('Computation of scale factors from position is not implemented yet. Please compute scale factor of {} on your own'
                                  .format(coord_nme)))
    ...
    for coord_nme in metric_pos_rename.keys():
        ...
        (point_position, tmp) = ax._get_axis_coord(domcfg[coord_nme])   # e.g. ('center', 'x_c')
        #print('\n (point_position, tmp) \n', (point_position, tmp))
        ...
        if point_position in ['inner', 'outer']:
            raise(NotImplementedError('The {} position has not been implemented yet'.format(point_position)))

        # cumsum of scale factors associated to the shift axis
        pos = grid.cumsum(domcfg[diff_shift_nme], axis=axis_nme, boundary='fill', fill_value=0)
...
def create_grid(domcfg):
    ...
    grid = xgcm.Grid(domcfg, periodic=False, metrics={('X',):['e1t','e1u'], ('Y',):['e2t','e2v'], ('Z',):['e3t_0','e3w_0'] })
    return grid
