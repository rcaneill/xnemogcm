...
def open_nemo(datadir, file_prefix='', domcfg=None, load_from_saved=True, save=True, saving_name=None):
    ...
    datadir = Path(datadir).expanduser() # expanduser replaces the '~' with '/home/$USER'
    ...
    if load_from_saved and saving_name.exists():
        ...
        filenames = [i for i in os.listdir(datadir)
                     if 'grid_' in i
                     and i[-3:] == '.nc'
                     and file_prefix in i]
        nemo_ds = xr.Dataset()
        for filename in filenames:
            ...
            for name in ds:
                ds[name].attrs['arakawa_point_type'] = point.point_type # adding metadata with point type
            ...
            nemo_ds = nemo_ds.merge(ds.drop_vars(['nav_lat', 'nav_lon','time_centered', 'time_centered_bounds','time_instant', 'time_instant_bounds'], errors='ignore'))
        # adding attributes
        ...
        # rename time
        nemo_ds = nemo_ds.rename({'time_counter':'t', 'time_counter_bounds':'t_bounds'})
